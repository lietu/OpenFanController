...
class RootHandler(BaseHandler):
    def get(self):
        ...
        resp = {
            "status": "ok",
            "message": "Karanovic Research Fan Controller API Server v0.1",
        }
        self.write(resp)
...
class FanProfile_List(BaseHandler):
    def get(self):
        ...
        self.send_response(
            status="ok",
            message=f"There are {len(profiles)} FAN profiles available.",
            data=profiles,
        )
    ...
    def post(self):
        ...
        try:
            ...
            return self.send_response(
                status="fail",
                message="Fan profile can only contain integer values!",
                data=None,
            )
        ...
        if len(fan_profile) != 10:
            return self.send_response(
                status="fail",
                message=f"Fan profile expects exactly 10 values! ({len(fan_profile)} given)",
                data=None,
            )
        ...
        return self.send_response(
            status="fail", message="Failed to add profile!", data=None
        )
...
class FanProfile_Remove(BaseHandler):
    def get(self):
        ...
        if name is None:
            return self.send_response(
                status="fail", message="Name can not be empty!", data=None
            )
        ...
        return self.send_response(
            status="ok", message="Profile does not exist", data=None
        )
...
class FanProfile_Set(BaseHandler):
    def get(self):
        ...
        if not profile:
            return self.send_response(
                status="fail",
                message="Profile does not exist! (Names are case-sensitive!)",
                data=None,
            )
        ...
        return self.send_response(
            status="ok", message=f"Profile `{name}` activated.{msg}.", data=None
        )
...
class FanSetALLPWM(BaseHandler):
    def get(self):
        ...
        return self.send_response(
            status="ok",
            message=f"Update queued. Setting ALL fans to PWM:{value}%",
            data=None,
        )
...
class FanSetPWM_Handler(BaseHandler):
    def get(self, fan_index=-1):
        if not self.is_valid_fan_index(fan_index):
            return self.send_response(
                status="fail",
                message=f"Invalid fan index (0<=`{fan_index}`<=9)",
                data=None,
            )
        ...
        return self.send_response(
            status="ok",
            message=f"Update queued. Setting fan #{fan_index} to PWM:{value}%",
            data=None,
        )
...
class FanSetRPM_Handler(BaseHandler):
    def get(self, fan_index=-1):
        ...
        return self.send_response(
            status="ok",
            message=f"Update queued. Fan:{fan_index} RPM:{value}",
            data=None,
        )
...
class FanAliasSet_Handler(BaseHandler):
    def get(self, fan_index=-1):
        ...
        if value is None:
            return self.send_response(
                status="fail", message="Fan alias can not be none!", data=None
            )
        if (
            re.search(r"^[a-z0-9\-_\.# ]*?$", value, re.MULTILINE | re.IGNORECASE)
            is None
        ):
            return self.send_response(
                status="fail",
                message="Fan alias can only contain `A-Z`, `0-9`, `-`, `_`, `#` and `<space>` characters!",
                data=None,
            )
        ...
        if self.config.set_fan_alias(fan_index, value):
            return self.send_response(
                status="ok",
                message=f"Fan #{fan_index} alias set to:`{value}`",
                data=None,
            )
        return self.send_response(
            status="fail",
            message=f"Failed to set fan alias. (`{fan_index}:{value}`)",
            data=None,
        )
...
class FAN_API_Service(Application):
    def __init__(self):
        ...
        if cfg_port:
            ...
            logger.info(
                f"Using COM port `{self.serialPort}` (specified in `config.yaml` file)."
            )
        elif env_port is not None:
            self.serialPort = env_port
            logger.info(
                f"Using COM port `{self.serialPort}` (specified in `OPENFANCOMPORT` env variable)."
            )
        else:
            self.serialPort = FanCommander.find_fan_controller()
            if self.serialPort is None:
                logger.error(
                    "Could not find Fan controller. Please make sure it's plugged in and (if necessary) drivers are installed."
                )
                raise Exception(
                    "Could not find Fan controller. Please make sure it's plugged in and (if necessary) drivers are installed."
                )
            ...
            logger.info(
                f"Using COM port `{self.serialPort}` (Found through USB VID:PID)."
            )
        ...
        self.handlers = [
            (
                r"/api/v0/profiles/list",
                FanProfile_List,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/profiles/add",
                FanProfile_Add,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/profiles/remove",
                FanProfile_Remove,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/profiles/set",
                FanProfile_Set,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
            ...
            (
                r"/api/v0/fan/all/set",
                FanSetALLPWM,
                {"handler": self.fan_commander, "config": self.config},
            ),
            # `/api/v0/fan/([0-9])/set` is now deprecated. Please use `/api/v0/fan/([0-9])/pwm`
            (
                r"/api/v0/fan/([0-9])/set",
                FanSetPWM_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/fan/([0-9])/pwm",
                FanSetPWM_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/fan/([0-9])/rpm",
                FanSetRPM_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/alias/all/get",
                FanAliasAll_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/alias/([0-9])/get",
                FanAliasGet_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
                r"/api/v0/alias/([0-9])/set",
                FanAliasSet_Handler,
                {"handler": self.fan_commander, "config": self.config},
            ),
            (
