...
class ConfigReader:
    ...
    def _load_config(self):
        if not os.path.exists(self.config_path):
            logger.error(
                f"Can not load config. Config file '{self.config_path}' does not exist!"
            )
        ...
        if server_cfg is False:
            ...
            logger.debug(
                "Server config missing from config file. Adding default values..."
            )
        ...
        if server_hw is False:
            ...
            config_data_missing = True
            logger.debug(
                "Hardware config missing from config file. Adding default values..."
            )
        ...
        if server_fp is False:
            ...
            config_data_missing = True
            logger.debug(
                "Fan profiles missing from config file. Adding default values..."
            )
        ...
        if fan_aliases is False:
            ...
            config_data_missing = True
            logger.debug(
                "Fan aliases missing from config file. Adding default values..."
            )
    ...
    def debug_config(self):
        ...
        logger.debug(
            f"\t Serial Timeout: {self.config['server']['communication_timeout']}"
        )
    ...
    def update_fan_profile(self, profile_name, profile_type, profile_params):
        ...
        if not any(profile_type in x for x in allowed_profile_types):
            logger.error(
                f"Fan profile type must be {allowed_profile_types}! (`{profile_type}` specified)"
            )
            return False
        ...
        self.config["fan_profiles"][profile_name] = {
            "type": profile_type,
            "values": profile_params,
        }
        self._save_config()
        return True
